...
```{r}
plt <- ggplot(fruits, aes(x = Fruit_length_avg, y = Fruit_width_avg, color = Shrub)) + 
  geom_point() + 
  geom_smooth(method = lm, se = FALSE, fullrange = TRUE)
plt
```

Are groups different? Let's also save this plot for later for modification in Illustrator.
```{r}
#| eval: false
ggsave("fruit_size_point_plot.pdf", plt, width = 20, height = 12)
```
